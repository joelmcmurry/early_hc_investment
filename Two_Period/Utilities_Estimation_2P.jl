...
function dgp_moments_par(initial_state_data, paramsprefs::ParametersPrefs, paramsdec::ParametersDec, paramsshock::ParametersShock;
  ...
  split_sim_choice_arg = sim_paths_split(sim_shocks[1], sim_shocks[2], sim_shocks[3][1], sim_shocks[4][1], paramsdec, paramsshock, par_N=par_N)
...
function smm(data_formatted, paramsprefs::ParametersPrefs, paramsdec::ParametersDec, paramsshock::ParametersShock;
  mu1_start=0.5, mu2_start=0.5, sigma1_start=0.1, sigma2_start=0.1, rho12_start=0.25,
  ...
  if opt_code == "neldermead"
    smm_opt = optimize(smm_obj_inner, [mu1_start, mu2_start, sigma1_start, sigma2_start, rho12_start,
...
function smm_sobol(data_formatted, paramsprefs::ParametersPrefs, paramsdec::ParametersDec, paramsshock::ParametersShock;
   sobol_N=10, mu1_lb=0.1, mu1_ub=1.5, mu2_lb=1.5, mu2_ub=0.9, sigma1_lb=0.01, sigma1_ub=1.0, sigma2_lb=0.01, sigma2_ub=1.,
   rho12_lb=-1., rho12_ub=1., eps_b_var_lb=0.01, eps_b_var_ub=1.,
   iota0_lb=-1., iota0_ub=1., iota1_lb=-1., iota1_ub=1., iota2_lb=-1., iota2_ub=1., iota3_lb=-1., iota3_ub=1.,
   ...
   s = SobolSeq(10, [mu1_lb, mu2_lb, sigma1_lb, sigma2_lb, rho12_lb, eps_b_var_lb,
   iota0_lb, iota1_lb, iota2_lb, iota3_lb], [mu1_ub, mu2_ub, sigma1_ub, sigma2_ub, rho12_ub, eps_b_var_ub,
    iota0_ub, iota1_ub, iota2_ub, iota3_ub])
   ...
   min_obj_val = Inf
   min_obj_params = zeros(10)

   # loop through Sobol sequence and compute objective function
   for i in 1:sobol_N
      ...
      if param_sobol[1] <= 0. || param_sobol[2] <= 0. || param_sobol[3] <= 0. || param_sobol[4] <= 0. ||
         param_sobol[5] < -1. || param_sobol[5] > 1. || param_sobol[6] <= 0.
         obj_val = Inf
      else
...
function smm_write_results(path, data_formatted, paramsprefs::ParametersPrefs, paramsdec::ParametersDec, paramsshock::ParametersShock;
  ...
  opt_trace=false, opt_iter=1000, opt_tol=1e-9, print_flag=true, par_flag=0, par_N=4)
   ...
   estimation_time = @elapsed estimation_result = smm(data_formatted, paramsprefs, paramsdec, paramsshock,
      mu1_start=mu1_start, mu2_start=mu2_start, sigma1_start=sigma1_start, sigma2_start=sigma2_start,
      rho12_start=rho12_start, eps_b_var_start=eps_b_var_start,
...
function smm_sobol_write_results(path, data_formatted, paramsprefs::ParametersPrefs, paramsdec::ParametersDec, paramsshock::ParametersShock;
   sobol_N=10, mu1_lb=0.1, mu1_ub=0.9, mu2_lb=0.1, mu2_ub=0.9, sigma1_lb=0.01, sigma1_ub=1.0, sigma2_lb=0.01, sigma2_ub=1.,
   rho12_lb=-1., rho12_ub=1., eps_b_var_lb=0.01, eps_b_var_ub=1.,
   iota0_lb=-1., iota0_ub=1., iota1_lb=-1., iota1_ub=1., iota2_lb=-1., iota2_ub=1., iota3_lb=-1., iota3_ub=1.,
   N=100, T=2, S=100, sample_code="nodraw", restrict_flag=1, seed=1234, error_log_flag=0, print_flag=false,
   par_flag=0, par_N=4)
  ...
  estimation_time = @elapsed estimation_result = smm_sobol(data_formatted, paramsprefs, paramsdec, paramsshock,
      sobol_N=sobol_N, mu1_lb=mu1_lb, mu1_ub=mu1_ub, mu2_lb=mu2_lb, mu2_ub=mu2_ub, sigma1_lb=sigma1_lb, sigma1_ub=sigma1_ub, sigma2_lb=sigma2_lb, sigma2_ub=sigma2_ub,
      rho12_lb=rho12_lb, rho12_ub=rho12_ub, eps_b_var_lb=eps_b_var_lb, eps_b_var_ub=eps_b_var_ub,
      iota0_lb=iota0_lb, iota0_ub=iota0_ub, iota1_lb=iota1_lb, iota1_ub=iota1_ub, iota2_lb=iota2_lb, iota2_ub=iota2_ub, iota3_lb=iota3_lb, iota3_ub=iota3_ub,
      N=N, T=T, S=S, sample_code=sample_code, restrict_flag=restrict_flag, seed=seed,
      error_log_flag=error_log_flag, print_flag=print_flag, par_flag=par_flag, par_N=4)
...
function smm_obj(initial_state_data, target_moments, W::Array, param_vec::Array,
  ...
  paramsprefs.mu[1], paramsprefs.mu[2], paramsprefs.sigma1, paramsprefs.sigma2,
    paramsprefs.rho12, paramsshock.eps_b_var,
  ...
  if param_vec[1] <= 0. || param_vec[2] <= 0. || param_vec[3] <= 0. || param_vec[4] <= 0. ||
    param_vec[5] < -1. || param_vec[5] > 1. || param_vec[6] <= 0.
     obj = Inf
  else
...
function smm_obj_testing(data_formatted, param_vec::Array,
  ...
  paramsprefs_float.mu[1], paramsprefs_float.mu[2], paramsprefs_float.sigma1, paramsprefs_float.sigma2,
    paramsprefs_float.rho12, paramsshock_float.eps_b_var,
