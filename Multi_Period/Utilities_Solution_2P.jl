...
function bellman_optim_child!(y::Float64, a::Float64, b::Float64,
  ...
  function utility(choices)
   ...
   if c > 0.0 && choices[1] >= 1.0 && choices[1] <= aprime_max
     ...
     for j in 1:paramsshock.eps_joint_discrete_N
       Vprime_array_exact[j] = ValueTerminal(y_b_array[j,1], choices[1], y_b_array[j,2], paramsdec)
     end

     value_out = u_t(c, b, paramsdec.alpha1, paramsdec.alpha2, paramsdec.alpha3) +
